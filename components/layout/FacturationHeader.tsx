'use client'

import { useState } from 'react'
import Link from 'next/link'
import LogoMark from '@/components/ui/LogoMark'
import { trackEvent } from '@/lib/analytics'

const navLinks = [
  { href: '#offre', label: "L'offre" },
  { href: '#parcours', label: 'Le parcours' },
  { href: '#pourquoi', label: 'Pourquoi nous' },
  { href: '#faq', label: 'FAQ' },
]

export default function FacturationHeader() {
  const [open, setOpen] = useState(false)
  const calendlyUrl = process.env.NEXT_PUBLIC_CALENDLY_URL ?? '/cabinet-growth-score'

  function handleCTA(source: string) {
    trackEvent('cta_click', { source, page: 'facturation-electronique-2026' })
  }

  return (
    <header
      className="sticky top-0 z-50 border-b border-line"
      style={{ background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }}
    >
      <div className="max-w-[1280px] mx-auto px-5 md:px-8 h-[68px] flex items-center justify-between">
        <Link
          href="/"
          className="inline-flex items-center gap-2.5 font-display font-bold text-ink leading-none"
          style={{ fontSize: 22, letterSpacing: '-0.035em' }}
          aria-label="GrowthCompta — accueil"
        >
          <LogoMark size={26} />
          <span>growthcompta</span>
          <span className="inline-block w-[7px] h-[7px] rounded-full bg-accent" style={{ transform: 'translateY(-2px)' }} />
        </Link>

        <nav className="hidden md:flex items-center gap-7" aria-label="Navigation page">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-sm text-ink-2 hover:text-ink transition-colors py-1.5"
            >
              {link.label}
            </a>
          ))}
        </nav>

        <div className="hidden md:flex items-center gap-2.5">
          <span className="inline-flex items-center gap-2 bg-paper border border-line px-3 py-1.5 rounded-full font-mono text-xs text-ink-2">
            <span className="w-1.5 h-1.5 rounded-full bg-accent" style={{ animation: 'pulse-dot 2.4s ease-in-out infinite' }} />
            Échéance 1er sept. 2026
          </span>
          <a
            href={calendlyUrl}
            target="_blank"
            rel="noopener noreferrer"
            onClick={() => handleCTA('header')}
            className="inline-flex items-center gap-2 bg-ink text-white font-sans font-medium text-sm px-[18px] py-2.5 rounded-full hover:opacity-90 transition-opacity group"
          >
            Diagnostic gratuit
            <span className="inline-block transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5">↗</span>
          </a>
        </div>

        <button
          onClick={() => setOpen(!open)}
          className="md:hidden p-2 rounded-lg text-ink hover:bg-paper transition-colors"
          aria-label={open ? 'Fermer le menu' : 'Ouvrir le menu'}
          aria-expanded={open}
        >
          <span className="block w-5 h-px bg-ink mb-1.5 transition-all" style={open ? { transform: 'rotate(45deg) translate(2px, 2px)' } : {}} />
          <span className="block w-5 h-px bg-ink mb-1.5" style={open ? { opacity: 0 } : {}} />
          <span className="block w-5 h-px bg-ink" style={open ? { transform: 'rotate(-45deg) translate(2px, -2px)' } : {}} />
        </button>
      </div>

      {open && (
        <div className="fixed inset-0 top-[68px] bg-white z-40 flex flex-col p-8 border-t border-line md:hidden">
          <nav className="flex flex-col">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                onClick={() => setOpen(false)}
                className="py-4 text-xl font-display font-semibold text-ink border-b border-line hover:text-accent transition-colors"
                style={{ letterSpacing: '-0.035em' }}
              >
                {link.label}
              </a>
            ))}
          </nav>
          <a
            href={calendlyUrl}
            target="_blank"
            rel="noopener noreferrer"
            onClick={() => { handleCTA('header_mobile'); setOpen(false) }}
            className="mt-8 inline-flex items-center justify-center gap-2 bg-ink text-white font-medium py-4 rounded-full hover:opacity-90 transition-opacity"
          >
            Diagnostic gratuit ↗
          </a>
        </div>
      )}
    </header>
  )
}
