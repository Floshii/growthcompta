...
export default function ResultsPage({ result, lead, onReset }: Props) {
  const { globalScore, categoryScores, levelConfig, quickWins, roadmap } = result

  const dimData = (Object.entries(categoryScores) as [string, number][]).map(([cat, score]) => {
    const max = CATEGORY_MAX[cat] ?? 10
    const pct = Math.round((score / max) * 100)
    return { cat, label: CATEGORY_LABELS[cat] ?? cat, short: cat.slice(0, 6).toUpperCase(), score, max, pct }
  })
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {[
                  ...
                  { label: 'Position', value: `Top ${Math.max(5, 100 - globalScore)}%` },
        ...
        <div className="max-w-[1280px] mx-auto px-5 md:px-8">
          <div className="mb-10">
            ...
            <p className="text-muted text-[15px] mt-3 max-w-[520px] leading-relaxed">
              Vos forces et angles morts sur les 8 dimensions de l&apos;acquisition.
            </p>
          </div>
            ...
            <div className="flex flex-col gap-2">
              {dimData.map((d, i) => (
                <div key={d.cat} className="flex items-center gap-3">
                  ...
                  <span className="text-[13px] text-ink flex-1 truncate">{d.label}</span>
        ...
        <div className="max-w-[1280px] mx-auto px-5 md:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {dimData.map(d => {
              ...
              return (
                <div key={d.cat} className="bg-paper rounded-xl p-5 border border-line">
      ...
      <section className="py-14 md:py-16">
        <div className="max-w-[1280px] mx-auto px-5 md:px-8">
          <div className="mb-10">
            ...
            <p className="text-muted text-[15px] mt-3 max-w-[520px] leading-relaxed">
              Actions sélectionnées selon vos angles morts — celles qui généreront le plus de valeur, le plus vite.
            </p>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {quickWins.slice(0, 3).map((qw, i) => (
              <div key={i} className="bg-white rounded-xl p-6 border border-line">
          ...
          <div className="hidden md:block bg-white/5 border border-white/10 rounded-2xl p-6">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              {['10:00', '14:30', '16:00'].map((t, i) => (
                <div key={i} className={`rounded-lg px-3 py-2 text-center font-mono text-[12px] ${i === 1 ? 'bg-accent text-white' : 'bg-white/8 text-white/60'}`}>
                  {t}
                </div>
              ))}
            </div>
